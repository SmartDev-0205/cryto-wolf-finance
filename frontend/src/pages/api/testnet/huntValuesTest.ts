...
import contractAbi from '../../../contracts/HuntingNFT.json';
...
const generateValuesHunt = async () => {
  ...
  for (const splittedSupply of splittedSupplies) {
    ...
    for (const supply of splittedSupply) {
      ...
      calls.push({ generated: await contract.methods.isGenerated(supply) });
    }
    ...
    // seed aleatoria para el método: generateResult
    const uuid = uuidv4();
    ...
    if (generated.length > 0) {
      ...
      // llamo al método write: generateResult
      try {
        // const resp =
        await contract.methods.generateResult(generated, randomSeed).send({
...
export default async function handleRequest(
  ...
  const message = `Generating hunting...`;

  await generateValuesHunt();
